...
DeepSeek_Web_To_API converts DeepSeek Web chat capability into OpenAI-compatible, Claude-compatible, and Gemini-compatible APIs. The core backend is Go-based, and the React WebUI admin panel lives in `webui/` (build output auto-generated to `static/admin` during deployment).
...
flowchart LR
    ...
    subgraph DeepSeek_Web_To_API["DeepSeek_Web_To_API 4.x (Modular HTTP Surface + PromptCompat Core)"]
...
- **Backend**: Go (`cmd/DeepSeek_Web_To_API/`, `internal/`), no Python runtime
- **Frontend**: React admin panel (`webui/`), served as static build at runtime
- **Deployment**: release binaries, Docker / GHCR, Zeabur, Linux systemd, local source run
...
| Unified CORS compatibility | `/v1/*`, `/anthropic/*`, `/v1beta/models/*`, and `/admin/*` share one CORS policy for third-party client preflight compatibility |
...
OpenAI `/v1/*` routes remain canonical, and DeepSeek_Web_To_API also accepts root shortcuts such as `/models`, `/chat/completions`, `/responses`, `/embeddings`, and `/files` for clients configured with the bare service URL.
...
Besides native IDs, DeepSeek_Web_To_API also accepts common aliases as input (for example `gpt-4.1`, `gpt-5`, `gpt-5-codex`, `o3`, `claude-*`, `gemini-*`), but `/v1/models` returns normalized DeepSeek native model IDs. The complete alias behavior is documented in [API.en.md](API.en.md#model-alias-resolution) and `config.example.json`.
...
- Set `ANTHROPIC_BASE_URL` to the DeepSeek_Web_To_API root URL (for example `http://127.0.0.1:5001`). Claude Code sends requests to `/v1/messages?beta=true`.
...
- If your environment has proxy variables, set `NO_PROXY=127.0.0.1,localhost,<your_host_ip>` for DeepSeek_Web_To_API to avoid proxy interception of local traffic.
- If tool calls are rendered as plain text and not executed, first verify the model output uses the recommended DSML block: `<|DSML|tool_calls><|DSML|invoke name="..."><|DSML|parameter name="...">...`. DeepSeek_Web_To_API also accepts legacy canonical XML: `<tool_calls><invoke name="..."><parameter name="...">...`; legacy `<tools>` / `<tool_call>` / `<tool_name>` / `<param>`, `<function_call>`, `tool_use`, or standalone JSON `tool_calls` are not executed.
...
`config.example.json.Annotation` mirrors the same structure and documents each
key's meaning, default, bounds, and optional environment override. It is for
maintainers and operators only; runtime still reads `config.json`.

Recommended per deployment mode:
- Local run: read `config.json` directly
- Docker / cloud platforms: generate Base64 from `config.json` and inject as `DEEPSEEK_WEB_TO_API_CONFIG_JSON`, or paste raw JSON directly
...
# Prepare the single runtime config
cp .env.example .env
cp config.example.json config.json
# Edit config.json (admin.key, admin.jwt_secret, keys, accounts, and runtime settings)
# Keep .env for deployment-layer overrides such as DEEPSEEK_WEB_TO_API_HOST_PORT or DEEPSEEK_WEB_TO_API_CONFIG_PATH.

# Start with compose
docker-compose up -d
```

The default `docker-compose.yml` uses `ghcr.io/meow-calculations/deepseek-web-to-api:latest` and maps host port `6011` to container port `5001`. If you want `5001` exposed directly, set `DEEPSEEK_WEB_TO_API_HOST_PORT=5001` (or adjust the `ports` mapping).
It also mounts `./config.json` to `/data/config.json` and sets `DEEPSEEK_WEB_TO_API_CONFIG_PATH=/data/config.json` by default, which avoids runtime token persistence failures caused by read-only `/app`.
...
2. After deployment, open `/admin` and login with `admin.key` from `config.json`.
...
> **WebUI auto-build**: On first local startup, if `static/admin` is missing, DeepSeek_Web_To_API will auto-run `npm ci` (only when dependencies are missing) and `npm run build -- --outDir static/admin --emptyOutDir` (requires Node.js). You can also build manually: `./scripts/build-webui.sh`
...
- `storage.chat_history_sqlite_path`: server-side chat history SQLite file; legacy `storage.chat_history_path` is only the first-run import source.
...
- `current_input_file`: the only active split mode; it is enabled by default and uploads the full context as a `DEEPSEEK_WEB_TO_API_HISTORY.txt` context file once the character threshold is reached.
...
For business endpoints (`/v1/*`, `/anthropic/*`, Gemini routes), DeepSeek_Web_To_API supports two modes:

| Mode | Description |
| --- | --- |
| **Managed account** | Use a key from `config.keys` via `Authorization: Bearer ...` or `x-api-key`; DeepSeek_Web_To_API auto-selects an account |
| **Direct token** | If the token is not in `config.keys`, DeepSeek_Web_To_API treats it as a DeepSeek token directly |

Optional header `X-DeepSeek-Web-To-API-Target-Account`: Pin a specific managed account (value is email or mobile).
Gemini routes also accept `x-goog-api-key`, or `?key=` / `?api_key=` when no auth header is present.

## Concurrency Model

```
Per-account inflight = DEEPSEEK_WEB_TO_API_ACCOUNT_MAX_INFLIGHT (default 2)
Recommended concurrency = account_count × per_account_inflight
Queue limit = DEEPSEEK_WEB_TO_API_ACCOUNT_MAX_QUEUE (default = recommended concurrency)
...
When `tools` is present in the request, DeepSeek_Web_To_API performs anti-leak handling:
...
This is for debugging issues such as Responses reasoning streaming and tool-call handoff. When enabled, DeepSeek_Web_To_API stores the latest N DeepSeek conversation payload pairs (request body + upstream response body), defaulting to 20 entries with auto-eviction; each response body is capped at 5 MB by default.
...
| [DEPLOY.md](docs/DEPLOY.md) / [DEPLOY.en.md](docs/DEPLOY.en.md) | Deployment guide (release binaries/Docker/Zeabur/systemd/local source) |
...
| [security-audit-2026-05-02.md](docs/security-audit-2026-05-02.md) | Security scan findings, fixes, and rerun commands |
...
- **Each binary archive includes**: the `deepseek-web-to-api` executable, `static/admin`, `config.example.json`, `.env.example`, `README.MD`, `README.en.md`, and `LICENSE`
