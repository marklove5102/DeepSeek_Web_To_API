...
cp .env.example .env
cp webui/.env.example webui/.env.development.local
```

`config.json` 是唯一运行配置源；`.env` 只放启动前必须存在的部署层覆盖项，`config.example.json.Annotation` 用来核对配置字段含义、默认值、范围和环境变量覆盖。
...
| 服务器端对话记录 | `internal/chathistory`、`internal/httpapi/historycapture`、`internal/httpapi/admin/history` |
...
- 协议捕获：OpenAI Chat 先看 `internal/httpapi/openai/chat/chat_history.go`；Claude / Responses 先看 `internal/httpapi/historycapture` 的接入点。
- 排队态捕获：外部 API 请求在账号池排队前会先写入 `queued` 历史；拿到账号后同一条记录更新为 `streaming` 并绑定 `account_id`，避免账号池等待数和历史列表活跃数不一致。
- 账号亲和溢出：同一会话优先复用绑定账号；如果该账号已达到 `runtime.account_max_inflight`（当前目标是 2），认证层会调用 `AcquireWaitPreferred` 溢出到其它空闲账号，而不是继续卡在原账号等待。
