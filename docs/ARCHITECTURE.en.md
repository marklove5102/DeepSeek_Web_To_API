# DeepSeek_Web_To_API Architecture & Project Layout
...
│   ├── DeepSeek_Web_To_API/                           # Main service bootstrap
│   └── DeepSeek_Web_To_API-tests/                     # E2E testsuite CLI bootstrap
...
│   ├── js/                               # Node-based compatibility test helpers
│   │   ├── chat-stream/                  # SSE parsing and token helpers
...
- `internal/js/chat-stream`: Node-side SSE parsing helpers used by compatibility fixtures and raw stream sample replay tooling.
...
- On first local startup, if `static/admin` is missing, DeepSeek_Web_To_API may auto-build it (Node.js required).
