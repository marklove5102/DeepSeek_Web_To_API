...
| ds2api 版本 | 对本文所有客户端（OpenClaw / Claude Code / Cline / Aider）的影响 |
|------------|---------------------------------------------------------------|
| **v1.0.8** | **Claude Code 用户必读**：`AutoDeleteRemoteSession` 已正式接入 `/v1/messages` handler deferred cleanup（Issue #20）。此前 Claude Code 通过该端点发起的请求即使 WebUI 开启"结束后全部删除"也不执行远程会话清理；v1.0.8 修复后，行为与 `/v1/chat/completions` 对齐。OpenClaw / Cline 同样走 `/v1/messages`，同等受益。 |
| **v1.0.10** | 严格模型 allowlist。若 OpenClaw / Cline 配置了非标准模型 id，请求返回 4xx。操作：在 ds2api WebUI Settings → Model Aliases 添加映射（热重载）。 |
| **v1.0.12** | 上游 429 弹性故障转移，峰值并发场景下 429 暴露率显著降低。 |
