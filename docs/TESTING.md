...
- 安全相关改动或发布前建议额外执行 `gosec ./...`、`govulncheck ./...`、`npm audit --prefix webui --audit-level=high --registry=https://registry.npmjs.org` 和敏感信息扫描；最近一次完整记录见 [security-audit-2026-05-02.md](./security-audit-2026-05-02.md)。
...
如果你只想跳过这些 preflight 检查，可以直接运行 `go run ./cmd/DeepSeek_Web_To_API-tests --no-preflight`。
...
| `--admin-key` | Admin 密钥 | `DEEPSEEK_WEB_TO_API_ADMIN_KEY` 环境变量，其次读取 `config.json` 的 `admin.key`，最后回退 `admin` |
...
| 位置 | 格式 |
| --- | --- |
| 请求头 | `X-DeepSeek-Web-To-API-Test-Trace: <trace_id>` |
...
- 当前测试产物目录和文件默认按敏感数据处理，运行时以 `0700` 目录和 `0600` 文件权限写入；如果复制到共享环境，请保持同等权限。
...
```bash
go run ./cmd/DeepSeek_Web_To_API-tests --no-preflight
```
...
```bash
go run ./cmd/DeepSeek_Web_To_API-tests \
  --out /tmp/deepseek-web-to-api-test \
  --timeout 60
```
