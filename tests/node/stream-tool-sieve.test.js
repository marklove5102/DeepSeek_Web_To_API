...
test('parseToolCalls keeps XML-looking CDATA content intact', () => {
  ...
  const payload = `<tool_calls><invoke name="Write"><parameter name="content"><![CDATA[${content}]]></parameter><parameter name="file_path">DeepSeek_Web_To_API-4.0-Release-Notes.md</parameter></invoke></tool_calls>`;
  const calls = parseToolCalls(payload, ['Write']);
  assert.equal(calls.length, 1);
  assert.equal(calls[0].input.content, content);
  assert.equal(calls[0].input.file_path, 'DeepSeek_Web_To_API-4.0-Release-Notes.md');
});
...
test('parseToolCalls rejects only-empty arguments', () => {
  const payload = '<tool_calls><invoke name="read_file"><parameter name="path">   </parameter><parameter name="optional"></parameter></invoke></tool_calls>';
  const calls = parseToolCalls(payload, ['read_file']);
  assert.equal(calls.length, 0);
});

test('sieve strips malformed executable-looking XML block', () => {
