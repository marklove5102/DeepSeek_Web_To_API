...
API_KEY="${5:-}"
ADMIN_KEY="${DEEPSEEK_WEB_TO_API_ADMIN_KEY:-admin}"
...
cleanup() {
  ...
  pkill -f "cmd/DeepSeek_Web_To_API" >/dev/null 2>&1 || true
}
trap cleanup EXIT

DEEPSEEK_WEB_TO_API_CONFIG_PATH="$CONFIG_PATH" \
DEEPSEEK_WEB_TO_API_ADMIN_KEY="$ADMIN_KEY" \
DEEPSEEK_WEB_TO_API_DEV_PACKET_CAPTURE=1 \
DEEPSEEK_WEB_TO_API_DEV_PACKET_CAPTURE_LIMIT=20 \
  go run ./cmd/DeepSeek_Web_To_API >/tmp/deepseek-web-to-api_capture_server.log 2>&1 &
...
for line in headers:
  if line.lower().startswith('x-deepseek-web-to-api-sample-dir:'):
...
headers=pathlib.Path(sys.argv[1]).read_text().splitlines()
for line in headers:
  if line.lower().startswith('x-deepseek-web-to-api-sample-id:'):
    print(line.split(':',1)[1].strip())
    raise SystemExit
print('')
PY
