...
function getDevBackendTarget(mode) {
    const env = loadEnv(mode, webuiDir, '')
    return (
        process.env.DEEPSEEK_WEB_TO_API_DEV_BACKEND ||
        process.env.VITE_DEEPSEEK_WEB_TO_API_BACKEND_URL ||
        env.DEEPSEEK_WEB_TO_API_DEV_BACKEND ||
        env.VITE_DEEPSEEK_WEB_TO_API_BACKEND_URL ||
        'http://localhost:5001'
    ).replace(/\/+$/, '')
}

export default defineConfig(({ mode }) => {
    const devBackendTarget = getDevBackendTarget(mode)
            ...
            proxy: {
                ...
                '/admin': {
                    ...
                    bypass(req) {
                        ...
                        console.log('[Vite Proxy] Proxy to backend:', url, '->', devBackendTarget)
