...
const LONG_RETENTION_LIMIT = 20_000
const LIMIT_OPTIONS = [0, 10, 20, 50, LONG_RETENTION_LIMIT]
const DISABLED_LIMIT = 0
const MESSAGE_COLLAPSE_AT = 700
const PAGE_SIZE = 100
const HISTORY_LIST_REFRESH_MS = 1000
const HISTORY_DETAIL_REFRESH_MS = 1000
const VIEW_MODE_KEY = 'deepseek-web-to-api_chat_history_view_mode'
...
const CURRENT_INPUT_FILE_PROMPT = 'Continue from the latest state in the attached DEEPSEEK_WEB_TO_API_HISTORY.txt context. Treat it as the current working state and answer the latest user request directly.'
...
function formatLimitLabel(limit, lang) {
    if (limit === LONG_RETENTION_LIMIT) {
        return lang === 'zh' ? '2w' : '20K'
    }
    return limit
}
...
function assistantOutputText(item) {
    if (!item) return ''
    const content = String(item.content || '').trim()
    if (content) return item.content
    const reasoning = String(item.reasoning_content || '').trim()
    if (reasoning && item.status !== 'error') return item.reasoning_content
    return ''
}
...
function ExpandableText({ text = '', threshold = MESSAGE_COLLAPSE_AT, expandLabel, collapseLabel, buttonClassName = 'text-primary hover:text-primary/80' }) {
...
function DetailConversation({ selectedItem, t, viewMode, detailScrollRef, assistantStartRef, bottomButtonClassName, onMessage }) {
    ...
    const assistantText = assistantOutputText(selectedItem)
    const reasoningAsOutput = !String(selectedItem.content || '').trim() && Boolean(String(selectedItem.reasoning_content || '').trim()) && selectedItem.status !== 'error'
        ...
        <>
            ...
            <div ref={assistantStartRef} className="flex gap-4 max-w-4xl mx-auto">
                ...
                <div className="space-y-4 flex-1 min-w-0">
                    {(selectedItem.reasoning_content || '').trim() && !reasoningAsOutput && (
                    ...
                    <div className="text-sm leading-7 text-foreground whitespace-pre-wrap break-words">
                        {selectedItem.status === 'error'
                            ...
                            : (assistantText || t('chatHistory.emptyAssistantOutput'))}
                    </div>
                </div>
            </div>
...
export default function ChatHistoryContainer({ authFetch, onMessage }) {
    ...
    const totalCount = Math.max(total, visibleCount)
    const listCountLabel = totalCount > visibleCount
        ? `${formatCount(visibleCount, lang)} / ${formatCount(totalCount, lang)}`
        : formatCount(totalCount, lang)
    ...
    const selectedIsLive = selectedSummary?.status === 'streaming' || selectedSummary?.status === 'queued' || selectedItem?.status === 'streaming' || selectedItem?.status === 'queued'

    const syncItems = (nextItems) => {
        setItems(nextItems)
        setSelectedId(prev => {
            if (!nextItems.length) return ''
            const liveItem = nextItems.find(item => item.status === 'streaming' || item.status === 'queued')
            const preferredId = liveItem?.id || nextItems[0].id
            if (autoFollowLatestRef.current) return preferredId
    ...
    const loadList = async ({ mode = 'silent', announceError = false, offset = 0, append = false } = {}) => {
        ...
        try {
            ...
            const res = await apiFetch(`/admin/chat-history?offset=${offset}&limit=${PAGE_SIZE}`, { headers })
    ...
    const loadMore = async () => {
        ...
        setLoadingMore(true)
        await loadList({ mode: 'silent', offset: items.length, append: true })
    ...
    useEffect(() => {
        ...
        }, HISTORY_LIST_REFRESH_MS)
        return () => window.clearInterval(timer)
    }, [autoRefreshReady, limit])

    useEffect(() => {
        if (!autoRefreshReady || !selectedId || !selectedIsLive) return undefined
        const timer = window.setInterval(() => {
            loadDetail(selectedId, { announceError: false })
        }, HISTORY_DETAIL_REFRESH_MS)
        return () => window.clearInterval(timer)
    }, [autoRefreshReady, selectedId, selectedIsLive])
    ...
    const handleLimitChange = async (nextLimit) => {
        ...
        try {
            ...
            listETagRef.current = ''
            setTotal(0)
            await loadList({ mode: 'silent', offset: 0, append: false })
            onMessage?.(
                'success',
                resolvedLimit === DISABLED_LIMIT
                    ? t('chatHistory.disabledSuccess')
                    : t('chatHistory.limitUpdated', { limit: formatLimitLabel(resolvedLimit, lang) })
            )
        } catch (error) {
    ...
    const handleSelectItem = (itemId, event) => {
        autoFollowLatestRef.current = itemId === items[0]?.id
        if (isMobileView) {
    ...
    const handleSelectItemKeyDown = (itemId, event) => {
        if (event.key !== 'Enter' && event.key !== ' ') return
        event.preventDefault()
        handleSelectItem(itemId, event)
    }
            ...
            <div className="rounded-2xl border border-border bg-card shadow-sm p-4 lg:p-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    {LIMIT_OPTIONS.map(option => (
                        ...
                        >
                            {option === DISABLED_LIMIT ? t('chatHistory.off') : formatLimitLabel(option, lang)}
                        </button>
                    ))}
                    <button
                        type="button"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[340px,minmax(0,1fr)] gap-6 h-[calc(100vh-240px)] min-h-[520px]">
                <div className="rounded-2xl border border-border bg-card shadow-sm min-h-0 overflow-hidden flex flex-col">
                    <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                        ...
                        <div className="text-xs text-muted-foreground tabular-nums">{listCountLabel}</div>
                    </div>
                    <div className="flex-1 overflow-y-auto p-3 space-y-3">
                        ...
                        {items.map(item => (
                            <div
                                ...
                                onKeyDown={(event) => handleSelectItemKeyDown(item.id, event)}
                                className={clsx(
