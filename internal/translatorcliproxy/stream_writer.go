...
func (w *OpenAIStreamTranslatorWriter) WriteHeader(statusCode int) {
	...
	// Defence-in-depth XSS mitigation. Stream content forwarded through
	// this wrapper carries upstream-LLM-generated bytes; the global
	// securityHeaders middleware already pins nosniff but we re-pin here
	// so CodeQL go/reflected-xss flow analysis sees the sanitiser at the
	// immediate write site.
	if w.dst.Header().Get("X-Content-Type-Options") == "" {
		w.dst.Header().Set("X-Content-Type-Options", "nosniff")
	}
