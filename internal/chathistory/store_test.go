...
func TestStoreCreatesAndPersistsEntries(t *testing.T) {
	...
	if started.Status != "queued" {
		t.Fatalf("expected queued status at start, got %#v", started.Status)
	}
	...
	if updated.AccountID != "user@example.com" {
		t.Fatalf("expected account binding to be updated, got %#v", updated.AccountID)
	}
...
func TestStoreDoesNotCacheCompletedDetailsOnReload(t *testing.T) {
	path := filepath.Join(t.TempDir(), "chat_history.json")
	store := New(path)

	entry, err := store.Start(StartParams{UserInput: "large request"})
	if err != nil {
		t.Fatalf("start entry failed: %v", err)
	}
	content := strings.Repeat("large answer ", 1024)
	...
	if got := len(store.details); got != 0 {
		t.Fatalf("completed detail should be released after save, cached %d entries", got)
	}

	reloaded := New(path)
	reloaded.mu.Lock()
	cachedOnLoad := len(reloaded.details)
	reloaded.mu.Unlock()
	if cachedOnLoad != 0 {
		t.Fatalf("completed details should not be eagerly loaded, cached %d entries", cachedOnLoad)
	}

	full, err := reloaded.Get(entry.ID)
	if err != nil {
		t.Fatalf("get detail failed: %v", err)
	}
	if full.Content != content {
		t.Fatalf("expected detail content to load on demand")
	}
	reloaded.mu.Lock()
	cachedAfterGet := len(reloaded.details)
	reloaded.mu.Unlock()
	if cachedAfterGet != 0 {
		t.Fatalf("read-only detail lookup should not retain completed detail, cached %d entries", cachedAfterGet)
	}
}
...
func TestStoreDeleteClearAndLimitValidation(t *testing.T) {
	...
	snapshot, err = store.SetLimit(MaxLimit)
	if err != nil {
		t.Fatalf("set max limit failed: %v", err)
	}
	if snapshot.Limit != MaxLimit {
		t.Fatalf("expected max limit %d, got %d", MaxLimit, snapshot.Limit)
	}
...
func TestStoreMarksUnfinishedEntriesStoppedOnLoad(t *testing.T) {
	path := filepath.Join(t.TempDir(), "chat_history.json")
	store := New(path)

	streaming, err := store.Start(StartParams{UserInput: "streaming"})
	if err != nil {
		t.Fatalf("start streaming failed: %v", err)
	}
	queued, err := store.Start(StartParams{Status: "queued", UserInput: "queued"})
	if err != nil {
		t.Fatalf("start queued failed: %v", err)
	}
	success, err := store.Start(StartParams{UserInput: "success"})
	if err != nil {
		t.Fatalf("start success failed: %v", err)
	}
	if _, err := store.Update(success.ID, UpdateParams{Status: "success", Content: "done", Completed: true}); err != nil {
		t.Fatalf("finish success failed: %v", err)
	}

	reloaded := New(path)
	for _, id := range []string{streaming.ID, queued.ID} {
		item, err := reloaded.Get(id)
		if err != nil {
			t.Fatalf("get unfinished item failed: %v", err)
		}
		if item.Status != "stopped" || item.FinishReason != "server_restart" || item.CompletedAt == 0 {
			t.Fatalf("expected unfinished item stopped after reload, got %#v", item)
		}
	}
	untouched, err := reloaded.Get(success.ID)
	if err != nil {
		t.Fatalf("get success item failed: %v", err)
	}
	if untouched.Status != "success" || untouched.Content != "done" {
		t.Fatalf("expected completed item unchanged, got %#v", untouched)
	}
}
