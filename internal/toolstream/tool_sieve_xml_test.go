...
func TestProcessToolSieveKeepsCDATAEmbeddedToolClosingBuffered(t *testing.T) {
	...
	chunks := []string{
		...
		payload[:innerClose],
		payload[innerClose:],
		"]]></parameter>\n    <parameter name=\"file_path\">DeepSeek_Web_To_API-4.0-Release-Notes.md</parameter>\n  </invoke>\n</tool_calls>",
	}
...
func TestProcessToolSieveStripsMalformedExecutableXMLBlock(t *testing.T) {
	var state State
	...
	if toolCalls != 0 {
		t.Fatalf("expected malformed executable-looking XML to be rejected, got %d events=%#v", toolCalls, events)
	}
	if textContent.String() != "" {
		t.Fatalf("expected malformed executable-looking XML not to leak, got %q", textContent.String())
	}
}
...
func TestProcessToolSieveFullwidthDSMLPrefixVariantDoesNotLeak(t *testing.T) {
	...
	chunks := []string{
		...
		"_calls>\n",
		"<|DSML|invoke name=\"Bash\">\n",
		"<|DSML|parameter name=\"command\"><![CDATA[ls -la /Users/aq/Desktop/myproject/DeepSeek_Web_To_API/]]></|DSML|parameter>\n",
		"<|DSML|parameter name=\"description\"><![CDATA[List project root contents]]></|DSML|parameter>\n",
		"</|DSML|invoke>\n",
		"<|DSML|invoke name=\"Bash\">\n",
		"<|DSML|parameter name=\"command\"><![CDATA[cat /Users/aq/Desktop/myproject/DeepSeek_Web_To_API/package.json 2>/dev/null || echo \"No package.json found\"]]></|DSML|parameter>\n",
...
func TestProcessToolSieveZeroWidthLowLineDSMLVariantDoesNotLeak(t *testing.T) {
	sep := "\u200d\u2581"
	var state State
	chunks := []string{
		"<\uff5cDSML" + sep + "tool_calls>\n",
	...
	if toolCalls != 2 {
		t.Fatalf("expected two tool calls from zero-width low-line DSML variant, got %d events=%#v", toolCalls, events)
	}
	if strings.Contains(strings.ToLower(textContent.String()), "dsml") ||
		strings.Contains(textContent.String(), "web_search") ||
		strings.Contains(textContent.String(), "tool_calls") {
		t.Fatalf("zero-width low-line DSML variant leaked to text: %q", textContent.String())
	}
}
