...
func findToolSegmentStart(state *State, s string) int {
	...
	for {
		...
		if isBareSingularToolCallTag(s, tag) {
			offset = tag.End + 1
			continue
		}
...
func isBareSingularToolCallTag(text string, tag toolcall.ToolMarkupTag) bool {
	if tag.DSMLLike {
		return false
	}
	if tag.Name == "tool_call" {
		return true
	}
	if tag.Name != "tool_calls" {
		return false
	}
	if tag.NameStart < 0 || tag.NameEnd > len(text) || tag.NameStart >= tag.NameEnd {
		return false
	}
	return strings.EqualFold(text[tag.NameStart:tag.NameEnd], "tool_call")
}
