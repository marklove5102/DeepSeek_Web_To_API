...
func (p *Pool) AcquireWaitPreferred(ctx context.Context, preferred string, exclude map[string]bool) (config.Account, bool) {
	if ctx == nil {
		ctx = context.Background()
	}
	exclude = normalizeExclude(exclude)
	for {
		...
		p.mu.Lock()
		if acc, ok := p.acquirePreferredLocked(preferred, exclude); ok {
			p.mu.Unlock()
			return acc, true
		}
		if !p.canQueueLocked("", exclude) {
			p.mu.Unlock()
			return config.Account{}, false
		}
		waiter := make(chan struct{})
...
func (p *Pool) acquirePreferredLocked(preferred string, exclude map[string]bool) (config.Account, bool) {
	if preferred != "" {
		if acc, ok := p.acquireLocked(preferred, exclude); ok {
			return acc, true
		}
	}
	return p.tryAcquire(exclude)
