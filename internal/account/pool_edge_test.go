...
func TestPoolEmptyNoAccounts(t *testing.T) {
	t.Setenv("DEEPSEEK_WEB_TO_API_ACCOUNT_MAX_INFLIGHT", "2")
	t.Setenv("DEEPSEEK_WEB_TO_API_ACCOUNT_MAX_QUEUE", "")
...
func TestPoolAcquireWaitPreferredOverflowsFromFullTarget(t *testing.T) {
	pool := newPoolForTest(t, "2")
	first, ok := pool.AcquireWait(context.Background(), "acc1@example.com", nil)
	if !ok {
		t.Fatal("expected first target acquire success")
	}
	second, ok := pool.AcquireWait(context.Background(), "acc1@example.com", nil)
	if !ok {
		t.Fatal("expected second target acquire success")
	}

	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	overflow, ok := pool.AcquireWaitPreferred(ctx, "acc1@example.com", nil)
	if !ok {
		t.Fatal("expected overflow acquire to use another account")
	}
	if overflow.Identifier() == "acc1@example.com" {
		t.Fatalf("expected overflow to avoid full preferred account, got %q", overflow.Identifier())
	}
	...
	pool.Release(overflow.Identifier())
}

func TestPoolMaxQueueSizeOverride(t *testing.T) {
	t.Setenv("DEEPSEEK_WEB_TO_API_ACCOUNT_MAX_INFLIGHT", "1")
	t.Setenv("DEEPSEEK_WEB_TO_API_ACCOUNT_MAX_QUEUE", "5")
	t.Setenv("DEEPSEEK_WEB_TO_API_CONFIG_JSON", `{"keys":["k1"],"accounts":[{"email":"acc1@example.com","token":"t1"}]}`)
	pool := NewPool(config.LoadStore())
	status := pool.Status()
