...
func TestDeepSeekRequestTotalTimeout(t *testing.T) {
	t.Setenv("DEEPSEEK_WEB_TO_API_HTTP_TOTAL_TIMEOUT_SECONDS", "")
	if got := NewClient(nil, nil).fallback.Timeout; got != 7200*time.Second {
		t.Fatalf("expected default 7200s, got %s", got)
	}

	t.Setenv("DEEPSEEK_WEB_TO_API_HTTP_TOTAL_TIMEOUT_SECONDS", "1800")
	if got := NewClient(nil, nil).fallback.Timeout; got != 1800*time.Second {
