...
func TestHandleNonStreamReturns429WhenUpstreamOutputEmpty(t *testing.T) {
	...
	h.handleNonStream(rec, resp, "cid-empty", "deepseek-v4-flash", "prompt", 0, false, false, false, nil, nil, nil)
...
func TestHandleNonStreamReturnsContentFilterErrorWhenUpstreamFilteredWithoutOutput(t *testing.T) {
	...
	h.handleNonStream(rec, resp, "cid-empty-filtered", "deepseek-v4-flash", "prompt", 0, false, false, false, nil, nil, nil)
...
func TestHandleNonStreamPromotesThinkingToolCallsWhenTextEmpty(t *testing.T) {
	...
	h.handleNonStream(rec, resp, "cid-thinking-tool", "deepseek-v4-pro", "prompt", 0, true, false, false, []string{"search"}, nil, nil)
...
func TestHandleNonStreamPromotesHiddenThinkingDSMLToolCallsWhenTextEmpty(t *testing.T) {
	...
	h.handleNonStream(rec, resp, "cid-hidden-thinking-tool", "deepseek-v4-pro", "prompt", 0, false, false, false, []string{"search"}, nil, nil)
	if rec.Code != http.StatusOK {
...
func TestFinishChatNonStreamRequiredToolChoiceViolation(t *testing.T) {
	h := &Handler{}
	rec := httptest.NewRecorder()

	h.finishChatNonStreamResult(rec, chatNonStreamResult{
		text:          "plain answer without a tool call",
		detectedCalls: 0,
		body:          map[string]any{"choices": []map[string]any{}},
		finishReason:  "stop",
	}, 0, "prompt", 0, true, nil)
...
func TestHandleStreamToolsPlainTextStreamsBeforeFinish(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid6", "deepseek-v4-flash", "prompt", 0, false, false, false, []string{"search"}, nil, nil)
...
func TestHandleStreamThinkingDisabledDoesNotLeakHiddenFragmentContinuations(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid-hidden-fragment", "deepseek-v4-flash", "prompt", 0, false, false, false, nil, nil, nil)
...
func TestHandleStreamEmitsSingleChoiceFramesForMultipleParsedParts(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid-multi-parts", "deepseek-v4-pro", "prompt", 0, true, true, false, nil, nil, nil)
...
func TestHandleStreamCoalescesSmallContentDeltas(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid-coalesce", "deepseek-v4-flash", "prompt", 0, false, false, false, nil, nil, nil)
...
func TestHandleStreamIncompleteCapturedToolJSONFlushesAsTextOnFinalize(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid10", "deepseek-v4-flash", "prompt", 0, false, false, false, []string{"search"}, nil, nil)
...
func TestChatStreamRuntimeRequiredToolChoiceViolationSuppressesPlainText(t *testing.T) {
	rec := httptest.NewRecorder()
	...
	runtime.text.WriteString("plain answer without a tool call")
	...
	if len(frames) != 1 {
		t.Fatalf("expected one error frame, got %#v body=%s", frames, rec.Body.String())
	}
	errObj, _ := frames[0]["error"].(map[string]any)
	if asString(errObj["code"]) != "tool_choice_violation" {
		t.Fatalf("expected code=tool_choice_violation, got %#v", frames[0])
	}
	if strings.Contains(rec.Body.String(), "plain answer without a tool call") {
		t.Fatalf("required tool_choice stream leaked plain text: %s", rec.Body.String())
	}
}
...
func TestHandleStreamPromotesThinkingToolCallsOnFinalizeWithoutMidstreamIntercept(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid-thinking-stream", "deepseek-v4-pro", "prompt", 0, true, false, false, []string{"search"}, nil, nil)
	...
	for _, frame := range frames {
		...
		for _, item := range choices {
			...
			if asString(delta["reasoning_content"]) != "" {
				t.Fatalf("did not expect tool-call markup to leak as reasoning_content, body=%s", rec.Body.String())
			}
		}
	}
...
func TestHandleStreamPromotesHiddenThinkingDSMLToolCallsOnFinalize(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid-hidden-thinking-stream", "deepseek-v4-pro", "prompt", 0, false, false, false, []string{"search"}, nil, nil)
...
func TestHandleStreamEmitsDistinctToolCallIDsAcrossSeparateToolBlocks(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid-multi", "deepseek-v4-flash", "prompt", 0, false, false, false, []string{"read_file", "search"}, nil, nil)
...
func TestHandleStreamCoercesSchemaDeclaredStringArgumentsOnFinalize(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid-string-protect", "deepseek-v4-flash", "prompt", 0, false, false, false, []string{"Write"}, toolsRaw, nil)
