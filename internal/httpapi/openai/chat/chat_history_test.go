...
type noAccountAfterCallerAuthStub struct{}

func (noAccountAfterCallerAuthStub) Determine(_ *http.Request) (*auth.RequestAuth, error) {
	return nil, auth.ErrNoAccount
}

func (noAccountAfterCallerAuthStub) DetermineCaller(_ *http.Request) (*auth.RequestAuth, error) {
	return &auth.RequestAuth{
		CallerID:      "caller:queued",
		TriedAccounts: map[string]bool{},
	}, nil
}

func (noAccountAfterCallerAuthStub) DetermineWithSession(_ *http.Request, _ []byte) (*auth.RequestAuth, error) {
	return nil, auth.ErrNoAccount
}

func (noAccountAfterCallerAuthStub) Release(_ *auth.RequestAuth) {}

func TestChatCompletionsRecordsQueuedHistoryWhenAccountUnavailable(t *testing.T) {
	historyStore := newTestChatHistoryStore(t)
	h := &Handler{
		Store:       mockOpenAIConfig{wideInput: true},
		Auth:        noAccountAfterCallerAuthStub{},
		DS:          streamStatusDSStub{},
		ChatHistory: historyStore,
	}

	reqBody := `{"model":"deepseek-v4-flash","messages":[{"role":"user","content":"wait for account"}],"stream":true}`
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(reqBody))
	...
	if len(snapshot.Items) != 1 {
		t.Fatalf("expected one queued history item, got %d", len(snapshot.Items))
	}
	item, err := historyStore.Get(snapshot.Items[0].ID)
	if err != nil {
		t.Fatalf("get queued history failed: %v", err)
	}
	if item.Status != "error" || item.StatusCode != http.StatusTooManyRequests || item.CallerID != "caller:queued" {
		t.Fatalf("unexpected queued failure history: %#v", item)
	}
	if item.UserInput != "wait for account" {
		t.Fatalf("expected original user input, got %#v", item.UserInput)
	}
}
...
func TestHandleStreamContextCancelledMarksHistoryStopped(t *testing.T) {
	...
	h.handleStream(rec, req, resp, "cid-stop", "deepseek-v4-flash", "prompt", 0, false, false, false, nil, nil, session)
...
type completionErrorDSStub struct {
	streamStatusDSStub
	err error
}

func (s completionErrorDSStub) CallCompletion(_ context.Context, _ *auth.RequestAuth, _ map[string]any, _ string, _ int) (*http.Response, error) {
	return nil, s.err
}

func TestChatCompletionsCompletionCancelMarksHistoryStopped(t *testing.T) {
	historyStore := newTestChatHistoryStore(t)
	h := &Handler{
		Store: mockOpenAIConfig{wideInput: true},
		Auth:  streamStatusAuthStub{},
		DS: completionErrorDSStub{
	...
	reqBody := `{"model":"deepseek-v4-flash","messages":[{"role":"user","content":"cancel me"}],"stream":false}`
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(reqBody))
	req.Header.Set("Authorization", "Bearer direct-token")
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	...
	if rec.Code != shared.StatusClientClosedRequest {
		t.Fatalf("expected 499, got %d body=%s", rec.Code, rec.Body.String())
	}
	item := singleChatHistoryItem(t, historyStore)
	if item.Status != "stopped" || item.StatusCode != http.StatusOK || item.FinishReason != "context_cancelled" {
		t.Fatalf("unexpected cancelled history item: %#v", item)
	}
}

func singleChatHistoryItem(t *testing.T, store *chathistory.Store) chathistory.Entry {
	t.Helper()
...
func TestChatCompletionsCurrentInputFilePersistsOriginalMessages(t *testing.T) {
	historyStore := newTestChatHistoryStore(t)
	...
	if ds.uploadCalls[0].Filename != "DEEPSEEK_WEB_TO_API_HISTORY.txt" {
		t.Fatalf("expected DEEPSEEK_WEB_TO_API_HISTORY.txt upload, got %q", ds.uploadCalls[0].Filename)
	}
	...
	if full.UserInput != "latest user turn" {
		t.Fatalf("expected latest original user input to be persisted, got %#v", full.UserInput)
	}
	if strings.Contains(full.Messages[len(full.Messages)-1].Content, "Answer the latest user request directly.") {
		t.Fatalf("expected history to keep original messages rather than neutral prompt, got %#v", full.Messages)
	}
}
