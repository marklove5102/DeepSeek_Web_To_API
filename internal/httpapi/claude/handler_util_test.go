...
func TestNormalizeClaudeMessagesToolResultNonTextPayloadStringified(t *testing.T) {
	...
	content, _ := m["content"].(string)
	if !containsStr(content, "image analysis") || !containsStr(content, `"type":"image"`) {
		t.Fatalf("expected non-text tool_result payload to be prompt-safe text, got %q", content)
	}
	if !containsStr(content, omittedBinaryMarker) {
...
func TestNormalizeClaudeMessagesHandlesClaudeCodeExtendedBlocks(t *testing.T) {
	msgs := []any{
		map[string]any{
			"role": "assistant",
			"content": []any{
				map[string]any{"type": "thinking", "thinking": "hidden plan"},
		...
		map[string]any{
			...
			"content": []any{
				map[string]any{
					...
					"name":        "ToolSearch",
					"content": []any{
						map[string]any{"type": "tool_reference", "tool_name": "mcp__repo__search"},
				...
				map[string]any{
					"type":  "cache_edits",
					"edits": []any{map[string]any{"type": "delete", "cache_reference": "toolu_old"}},
				},
			},
		},
	}
	...
	assistant, _ := got[0].(map[string]any)
	assistantContent, _ := assistant["content"].(string)
	if !containsStr(assistantContent, "[thinking]") || !containsStr(assistantContent, "hidden plan") || !containsStr(assistantContent, "visible answer") {
		t.Fatalf("expected thinking and text to be represented in assistant prompt, got %q", assistantContent)
	}
	toolMsg, _ := got[1].(map[string]any)
	toolContent, _ := toolMsg["content"].(string)
	if !containsStr(toolContent, "[Tool reference loaded: mcp__repo__search]") || !containsStr(toolContent, "Tool loaded.") {
		t.Fatalf("expected tool_reference to become prompt-safe text, got %q", toolContent)
	}
	if containsStr(toolContent, "cache_control") || containsStr(toolContent, "cache_edits") {
		t.Fatalf("expected Claude cache transport blocks stripped, got %q", toolContent)
	}
}

func TestNormalizeClaudeMessagesEmptyToolResultGetsPlaceholder(t *testing.T) {
	msgs := []any{
		map[string]any{
			"role": "user",
			"content": []any{
				map[string]any{"type": "tool_result", "tool_use_id": "toolu_empty", "name": "Bash", "content": ""},
			},
		},
	}
	...
	toolMsg, _ := got[0].(map[string]any)
	if toolMsg["content"] != "(Bash completed with no output)" {
		t.Fatalf("expected empty tool result placeholder, got %#v", toolMsg)
	}
}
