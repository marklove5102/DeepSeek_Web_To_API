...
func TestHandleClaudeStreamRealtimeStreamsTextWhenToolsArePresent(t *testing.T) {
	h := &Handler{}
	resp := makeClaudeSSEHTTPResponse(
		`data: {"p":"response/content","v":"Hel"}`,
		`data: {"p":"response/content","v":"lo"}`,
	...
	h.handleClaudeStreamRealtime(rec, req, resp, "claude-sonnet-4-5", []any{map[string]any{"role": "user", "content": "hi"}}, false, false, []string{"Bash", "Read"}, nil)
	...
	if textDeltas < 2 {
		t.Fatalf("expected text to stream in multiple deltas with tools present, got %d body=%s", textDeltas, rec.Body.String())
	}
	if combined.String() != "Hello" {
		t.Fatalf("unexpected combined text: %q body=%s", combined.String(), rec.Body.String())
	}
}

func TestHandleClaudeStreamRealtimeSuppressesSplitToolResultTokenLeak(t *testing.T) {
	h := &Handler{}
	resp := makeClaudeSSEHTTPResponse(
		`data: {"p":"response/content","v":"现在并行执行修复操作：\n\n<｜tool_result｜>account.json"}`,
		`data: {"p":"response/content","v":".bak deleted<|end▁f▁of▁tool_result｜>\n<｜tool_result/>user.txt deleted<｜end▁f▁of▁sentence｜>\n完成"}`,
		`data: [DONE]`,
	)
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/anthropic/v1/messages", nil)
	...
	if strings.Contains(got, "tool_result") || strings.Contains(got, "deleted") || strings.Contains(got, "end▁") {
		t.Fatalf("expected leaked tool_result blocks to be suppressed, got %q body=%s", got, rec.Body.String())
	}
	if !strings.Contains(got, "现在并行执行修复操作：") || !strings.Contains(got, "完成") {
		t.Fatalf("expected surrounding visible text to remain, got %q body=%s", got, rec.Body.String())
	}
}
...
func TestHandleClaudeStreamRealtimeDetectsToolUseWithLeadingProse(t *testing.T) {
	...
	for _, f := range findClaudeFrames(frames, "content_block_delta") {
		delta, _ := f.Payload["delta"].(map[string]any)
		if delta["type"] == "text_delta" {
			combinedText.WriteString(asString(delta["text"]))
		}
	}
	if got := combinedText.String(); !strings.Contains(got, "I'll call a tool now.") || strings.Contains(got, "<tool_calls") {
		t.Fatalf("expected leading prose to stream without tool markup, got %q body=%s", got, rec.Body.String())
	}
