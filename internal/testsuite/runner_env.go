...
func (r *Runner) prepareRunDir() error {
	...
	if err := os.MkdirAll(filepath.Join(r.runDir, "cases"), 0o700); err != nil {
		return err
	}
...
func (r *Runner) runPreflight(ctx context.Context) error {
	...
	f, err := os.OpenFile(r.preflightLog, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o600)
	if err != nil {
		return err
	}
	...
	for _, step := range steps {
		...
		// #nosec G204 -- preflight steps are fixed literals from preflightSteps.
...
func (r *Runner) prepareConfigIsolation() error {
	...
	// #nosec G304 -- the live testsuite reads the operator-supplied local config path.
	...
	// #nosec G703 -- configCopyPath is always inside the private per-run tmpDir.
	if err := os.WriteFile(r.configCopyPath, raw, 0o600); err != nil {
		return err
	}
	var cfg runConfig
...
func (r *Runner) startServer(ctx context.Context) error {
	...
	logFd, err := os.OpenFile(r.serverLog, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o600)
	if err != nil {
		return err
	}
	r.serverLogFd = logFd
	cmd := exec.CommandContext(ctx, "go", "run", "./cmd/DeepSeek_Web_To_API")
	cmd.Stdout = logFd
	cmd.Stderr = logFd
	cmd.Env = prepareServerEnv(os.Environ(), map[string]string{
		"PORT":                                 strconv.Itoa(port),
		"DEEPSEEK_WEB_TO_API_CONFIG_PATH":      r.configCopyPath,
		"DEEPSEEK_WEB_TO_API_AUTO_BUILD_WEBUI": "false",
		"DEEPSEEK_WEB_TO_API_CONFIG_JSON":      "",
	})
