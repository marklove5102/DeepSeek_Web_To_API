...
func TestRepairOpenAIToolMessagesBackfillsMissingToolCallID(t *testing.T) {
	raw := []any{
		map[string]any{"role": "user", "content": "查资料"},
		map[string]any{
			"role": "assistant",
			"tool_calls": []any{
				map[string]any{
					...
					"function": map[string]any{
						"name":      "lookup",
						"arguments": map[string]any{"query": "deepseek-web-to-api"},
					},
				},
			},
		},
	...
	toolMsg, ok := repaired[2].(map[string]any)
	if !ok {
		t.Fatalf("expected repaired tool message, got %#v", repaired[2])
	}
	if toolMsg["tool_call_id"] != "call_lookup" {
		t.Fatalf("expected missing tool_call_id backfilled, got %#v", toolMsg)
	}

	normalized := NormalizeOpenAIMessagesForPrompt(raw, "")
	assistantContent, _ := normalized[1]["content"].(string)
	if !strings.Contains(assistantContent, `<|DSML|parameter name="query">`) {
		t.Fatalf("expected object arguments normalized into prompt parameters, got %q", assistantContent)
	}
	if normalized[2]["role"] != "tool" || !strings.Contains(normalized[2]["content"].(string), "命中一条结果") {
		t.Fatalf("expected repaired tool result preserved, got %#v", normalized[2])
	}
}

func TestRepairOpenAIToolMessagesConvertsWrongUserRoleToolResult(t *testing.T) {
	raw := []any{
		map[string]any{
			"role": "assistant",
			"tool_calls": []any{
	...
	if !ok {
		t.Fatalf("expected repaired message map, got %#v", repaired[1])
	}
	if toolMsg["role"] != "tool" {
		t.Fatalf("expected wrong user role converted to tool, got %#v", toolMsg)
	}
	if toolMsg["tool_call_id"] != "call_read" {
		t.Fatalf("expected single pending call id to replace mismatched id, got %#v", toolMsg)
	}
}

func TestRepairOpenAIToolMessagesHandlesAnthropicStyleBlocks(t *testing.T) {
	raw := []any{
		map[string]any{
			"role": "assistant",
			"content": []any{
				map[string]any{"type": "text", "text": "我先读取文件"},
				map[string]any{
					"type":  "tool_use",
					"id":    "toolu_1",
					"name":  "read_file",
					"input": map[string]any{"path": "docs/prompt-compatibility.md"},
				},
			},
		},
		map[string]any{
	...
	if !ok {
		t.Fatalf("expected assistant map, got %#v", repaired[0])
	}
	if assistant["role"] != "assistant" || assistant["tool_calls"] == nil {
		t.Fatalf("expected Anthropic tool_use converted to assistant tool_calls, got %#v", assistant)
	}
	toolMsg, ok := repaired[1].(map[string]any)
	if !ok {
		t.Fatalf("expected tool result map, got %#v", repaired[1])
	}
	if toolMsg["role"] != "tool" || toolMsg["tool_call_id"] != "toolu_1" {
		t.Fatalf("expected Anthropic tool_result converted to tool message, got %#v", toolMsg)
	}

	normalized := NormalizeOpenAIMessagesForPrompt(raw, "")
	if len(normalized) != 2 {
		t.Fatalf("expected two normalized messages, got %#v", normalized)
	}
	if !strings.Contains(normalized[0]["content"].(string), "我先读取文件") ||
		!strings.Contains(normalized[0]["content"].(string), `<|DSML|invoke name="read_file">`) {
