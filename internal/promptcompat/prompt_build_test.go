...
func TestBuildOpenAIFinalPrompt_StreamPathKeepsFinalAnswerInstruction(t *testing.T) {
	messages := []any{
	...
	if !strings.Contains(finalPrompt, "Remember: The ONLY valid way to use tools is the <|DSML|tool_calls>...</|DSML|tool_calls> block at the end of your response.") {
		t.Fatalf("stream finalPrompt missing final tool-call anchor instruction: %q", finalPrompt)
	}
	if !strings.Contains(finalPrompt, "TOOL CALL FORMAT") {
		t.Fatalf("stream finalPrompt missing xml format instruction: %q", finalPrompt)
	}
	if !strings.Contains(finalPrompt, "Do NOT wrap XML in markdown fences") {
		t.Fatalf("stream finalPrompt missing no-fence xml instruction: %q", finalPrompt)
	}
	if strings.Contains(finalPrompt, "```json") {
		t.Fatalf("stream finalPrompt should not require fenced tool calls: %q", finalPrompt)
	}
}
