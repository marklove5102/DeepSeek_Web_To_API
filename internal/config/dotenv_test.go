...
func TestLoadDotEnvLoadsWorkingDirectoryFileWithoutOverridingExistingEnv(t *testing.T) {
	...
	const newKey = "DEEPSEEK_WEB_TO_API_TEST_DOTENV_NEW"
	const keepKey = "DEEPSEEK_WEB_TO_API_TEST_DOTENV_KEEP"
	const quotedKey = "DEEPSEEK_WEB_TO_API_TEST_DOTENV_QUOTED"

	unsetEnv(t, newKey)
	unsetEnv(t, quotedKey)
	t.Setenv(keepKey, "from-env")

	content := "DEEPSEEK_WEB_TO_API_TEST_DOTENV_NEW=from-file\n" +
		"DEEPSEEK_WEB_TO_API_TEST_DOTENV_KEEP=from-file\n" +
		"DEEPSEEK_WEB_TO_API_TEST_DOTENV_QUOTED=\"line1\\nline2\"\n"
...
func TestLoadDotEnvStripsInlineCommentsFromUnquotedValues(t *testing.T) {
	...
	const plainKey = "DEEPSEEK_WEB_TO_API_TEST_DOTENV_PLAIN"
	const hashKey = "DEEPSEEK_WEB_TO_API_TEST_DOTENV_HASH"
	const quotedKey = "DEEPSEEK_WEB_TO_API_TEST_DOTENV_QUOTED_COMMENT"
	const exportKey = "DEEPSEEK_WEB_TO_API_TEST_DOTENV_EXPORT"
...
func TestApplyLegacyEnvAliasesMapsOldPrefix(t *testing.T) {
	const oldKey = "DS2API_TEST_ALIAS_KEY"
	const newKey = "DEEPSEEK_WEB_TO_API_TEST_ALIAS_KEY"
	t.Setenv(oldKey, "legacy-value")
	unsetEnv(t, newKey)

	ApplyLegacyEnvAliases()

	if got := os.Getenv(newKey); got != "legacy-value" {
		t.Fatalf("expected legacy alias to populate new env, got %q", got)
	}
}

func TestApplyLegacyEnvAliasesKeepsNewPrefixPrecedence(t *testing.T) {
	const oldKey = "DS2API_TEST_ALIAS_PRECEDENCE"
	const newKey = "DEEPSEEK_WEB_TO_API_TEST_ALIAS_PRECEDENCE"
	t.Setenv(oldKey, "legacy-value")
	...
	if got := os.Getenv(newKey); got != "new-value" {
		t.Fatalf("expected new env to keep precedence, got %q", got)
	}
}
