...
func TestSwitchAccountWaitsForBusyFallbackAccount(t *testing.T) {
	t.Setenv("DEEPSEEK_WEB_TO_API_CONFIG_JSON", `{
		"keys":["managed-key"],
	...
	busy, ok := pool.Acquire("acc2@test.com", nil)
	if !ok {
		t.Fatal("expected to occupy fallback account")
	}
	defer pool.Release(busy.Identifier())
	...
	go func() {
		done <- r.SwitchAccount(ctx, a)
	}()
	time.Sleep(20 * time.Millisecond)
	pool.Release(busy.Identifier())
	if !<-done {
		t.Fatal("expected switch to wait and then succeed")
	}
	if a.AccountID != "acc2@test.com" {
		t.Fatalf("expected fallback account, got %q", a.AccountID)
	}
}
...
func TestVerifyAdminRequestInvalidCredentials(t *testing.T) {
	t.Setenv("DEEPSEEK_WEB_TO_API_ADMIN_KEY", "correct-key")
	req, _ := http.NewRequest("GET", "/admin/config", nil)
...
func TestDetermineWithTargetAccount(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", "/", nil)
	req.Header.Set("Authorization", "Bearer managed-key")
	req.Header.Set("X-DeepSeek-Web-To-API-Target-Account", "acc2@test.com")
	a, err := r.Determine(req)
	if err != nil {
		t.Fatalf("determine failed: %v", err)
	}
