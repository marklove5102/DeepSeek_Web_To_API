...
| 协议响应缓存 | OpenAI / Claude / Gemini 兼容 POST 协议默认启用：内存缓存 5 分钟且最多 3.8GB，gzip 磁盘缓存 4 小时且最多 16GB；命中时返回 `X-DeepSeek-Web-To-API-Cache: memory|disk` 与 `X-DeepSeek-Web-To-API-Cache-Expires-At` |
| CORS | 已启用（统一覆盖 `/v1/*`、`/anthropic/*`、`/v1beta/models/*`、`/admin/*`；浏览器有 `Origin` 时回显该 Origin，否则为 `*`；默认允许 `Content-Type`, `Authorization`, `X-API-Key`, `X-DeepSeek-Web-To-API-Target-Account`, `X-DeepSeek-Web-To-API-Source`, `X-Goog-Api-Key`, `Anthropic-Version`, `Anthropic-Beta`，并会放行预检里声明的第三方请求头，如 `x-stainless-*`；内部专用头 `X-DeepSeek-Web-To-API-Internal-Token` 仍被拦截） |
...
- 协议响应缓存位于路由中间件层，覆盖 OpenAI Chat/Responses/Embeddings、Claude/Anthropic Messages/CountTokens、Gemini GenerateContent/StreamGenerateContent；缓存键按调用方、协议路径、查询参数、影响输出的请求头和请求体摘要隔离。
...
- 默认缓存范围：`POST /v1/chat/completions`、`POST /v1/responses`、`POST /v1/embeddings`、Claude/Anthropic Messages 与 CountTokens、Gemini `generateContent` / `streamGenerateContent`。
- 内存层保存 5 分钟，最多占用 3.8GB；磁盘层保存 4 小时，最多占用 16GB，使用 gzip 压缩，默认目录为 `data/response_cache`，推荐通过 `config.json` 的 `cache.response.dir` 调整，`DEEPSEEK_WEB_TO_API_RESPONSE_CACHE_DIR` 仅作为部署层覆盖。
- 命中缓存时不会再请求 DeepSeek，上游响应体会按原协议形态回放，并附加 `X-DeepSeek-Web-To-API-Cache: memory|disk` 与 `X-DeepSeek-Web-To-API-Cache-Expires-At`。
- 请求头 `Cache-Control: no-cache` / `Cache-Control: no-store` 或 `X-DeepSeek-Web-To-API-Cache-Control: bypass` 会绕过缓存。
...
# 编辑 config.json（admin、keys、accounts、server、storage、cache）
```

按部署方式使用：

- 本地运行：直接读取 `config.json`
- Docker / 云平台：从 `config.json` 生成 Base64，填入 `DEEPSEEK_WEB_TO_API_CONFIG_JSON`，也可以直接填原始 JSON
- 服务器端对话历史运行态写入 `storage.chat_history_sqlite_path`（默认 `data/chat_history.sqlite`）；旧 `storage.chat_history_path` 仅作为首次启动时的 JSON 导入来源。

```bash
DEEPSEEK_WEB_TO_API_CONFIG_JSON="$(base64 < config.json | tr -d '\n')"
```

云平台部署建议在部署前生成 `DEEPSEEK_WEB_TO_API_CONFIG_JSON` 并填入环境变量；部署后 `/admin` 仍可导入配置用于当前实例运行态。
...
**可选请求头**：`X-DeepSeek-Web-To-API-Target-Account: <email_or_mobile>` — 指定使用某个托管账号；如果目标账号不存在，或管理账号队列已耗尽，相关业务请求会返回 `429`，当前不会附带 `Retry-After` 头。若账号存在但登录/刷新失败，则返回对应的 `401` 或上游错误。
...
| GET | `/admin/metrics/overview` | Admin | 总览页运行指标、缓存指标与成功率口径 |
| GET | `/admin/version` | Admin | 查询当前自定义构建版本 |

OpenAI `/v1/*` 仍是规范路径。对于只配置 DeepSeek_Web_To_API 根地址的客户端，同一套 OpenAI handler 也通过根路径快捷路由暴露：`/models`、`/models/{id}`、`/chat/completions`、`/responses`、`/responses/{response_id}`、`/embeddings`、`/files`。
...
当请求中含 `tools` 时，DeepSeek_Web_To_API 做防泄漏处理：
...
读取总览页运行指标，包含 `throughput`、`tokens`、`cost`、`host`、`cache`、`history` 与 `pool`。

- `cache.cacheable_hit_rate` / `cache.cacheable_miss_rate` 只统计可缓存请求，避免上游失败、客户端取消等不可缓存响应污染缓存判断。
- `cache.hit_rate` / `cache.miss_rate` 保留为总查询口径，包含不可缓存失败，仅用于排障。
- `history.success_rate` 计算成功率时排除用户侧 `401 / 403 / 502 / 504 / 524`，这些状态计入 `history.excluded_from_failure_rate`，不进入失败率分母。
...
`replace` 模式会按完整配置结构替换；`merge` 模式会合并 `keys`、`api_keys`、`accounts`、`model_aliases`，并覆盖 `admin`、`runtime`、`responses`、`embeddings` 中的非空字段。`compat`、`auto_delete`、`current_input_file` 建议通过 `/admin/settings` 或配置文件管理；`history_split` 仅保留为旧配置兼容字段；`toolcall` 相关字段会被忽略。
...
查询当前自定义构建版本；不会再请求官方 GitHub Release，更新策略由当前维护者自行决定：
