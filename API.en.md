...
| Protocol response cache | Enabled for OpenAI / Claude / Gemini-compatible POST protocols: 5-minute memory cache capped at 3.8GB plus 4-hour gzip disk cache capped at 16GB; cache hits return `X-DeepSeek-Web-To-API-Cache: memory|disk` and `X-DeepSeek-Web-To-API-Cache-Expires-At` |
| CORS | Enabled (uniformly covers `/v1/*`, `/anthropic/*`, `/v1beta/models/*`, and `/admin/*`; echoes the browser `Origin` when present, otherwise `*`; default allow-list includes `Content-Type`, `Authorization`, `X-API-Key`, `X-DeepSeek-Web-To-API-Target-Account`, `X-DeepSeek-Web-To-API-Source`, `X-Goog-Api-Key`, `Anthropic-Version`, `Anthropic-Beta`, and also accepts third-party preflight-requested headers such as `x-stainless-*`; internal-only `X-DeepSeek-Web-To-API-Internal-Token` remains blocked) |
...
- Protocol response caching sits at the router middleware layer and covers OpenAI Chat/Responses/Embeddings, Claude/Anthropic Messages/CountTokens, and Gemini GenerateContent/StreamGenerateContent. Cache keys are isolated by caller, protocol path, query string, output-affecting headers, and request body digest.
...
- Tool-calling semantics are aligned between Go and Node runtime: models should output the DSML shell `<|DSML|tool_calls>` → `<|DSML|invoke name="...">` → `<|DSML|parameter name="...">`; DeepSeek_Web_To_API also accepts legacy canonical XML `<tool_calls>` → `<invoke name="...">` → `<parameter name="...">`. DSML is normalized back to XML at the parser entry, so internal parsing remains XML-based, with stream-time anti-leak filtering.
...
### Protocol Response Cache

- Cached by default: `POST /v1/chat/completions`, `POST /v1/responses`, `POST /v1/embeddings`, Claude/Anthropic Messages and CountTokens, plus Gemini `generateContent` / `streamGenerateContent`.
- Memory entries live for 5 minutes and are capped at 3.8GB; disk entries live for 4 hours, are gzip-compressed, capped at 16GB, and default to `data/response_cache`. Prefer `cache.response.dir` in `config.json`; keep `DEEPSEEK_WEB_TO_API_RESPONSE_CACHE_DIR` only as a deployment-layer override.
- On a cache hit DeepSeek_Web_To_API skips DeepSeek and replays the original protocol-shaped response with `X-DeepSeek-Web-To-API-Cache: memory|disk` and `X-DeepSeek-Web-To-API-Cache-Expires-At`.
- `Cache-Control: no-cache`, `Cache-Control: no-store`, or `X-DeepSeek-Web-To-API-Cache-Control: bypass` bypasses the cache.
...
# Edit config.json (admin, keys, accounts, server, storage, cache)
```

Use it per deployment mode:

- Local run: read `config.json` directly
- Docker / cloud platforms: generate Base64 from `config.json`, then set `DEEPSEEK_WEB_TO_API_CONFIG_JSON`, or paste raw JSON directly
- Server-side chat history is written to `storage.chat_history_sqlite_path` by default (`data/chat_history.sqlite`); legacy `storage.chat_history_path` is only used as the first-run JSON import source.

```bash
DEEPSEEK_WEB_TO_API_CONFIG_JSON="$(base64 < config.json | tr -d '\n')"
```

For cloud deployments, generate and set `DEEPSEEK_WEB_TO_API_CONFIG_JSON` before deployment when possible. After deployment, `/admin` can still import configuration for the current runtime.
...
| GET | `/admin/metrics/overview` | Admin | Overview runtime, cache, and success-rate metrics |
| GET | `/admin/version` | Admin | Check current custom build version |

OpenAI `/v1/*` paths are canonical. For clients configured with the bare DeepSeek_Web_To_API service URL, the same OpenAI handlers are also exposed through root shortcuts: `/models`, `/models/{id}`, `/chat/completions`, `/responses`, `/responses/{response_id}`, `/embeddings`, and `/files`.
...
For `chat` / `responses` / `embeddings`, DeepSeek_Web_To_API follows a wide-input/strict-output policy:
...
When `tools` is present, DeepSeek_Web_To_API performs anti-leak handling:
...
**Stream**: Once high-confidence toolcall features are matched, DeepSeek_Web_To_API emits `delta.tool_calls` immediately (without waiting for full argument closure), then keeps sending argument deltas; confirmed tool-call fragments are not forwarded as `delta.content`.
...
If `tool_choice=required` and no valid tool call is produced, DeepSeek_Web_To_API returns HTTP `422` (`error.code=tool_choice_violation`).
...
If `tool_choice=required` is violated in stream mode, DeepSeek_Web_To_API emits `response.failed` then `[DONE]` (no `response.completed`).
...
Besides `/anthropic/v1/*`, DeepSeek_Web_To_API also supports shortcut paths: `/v1/messages`, `/messages`, `/v1/messages/count_tokens`, `/messages/count_tokens`.
...
> `anthropic-version` is optional; DeepSeek_Web_To_API auto-fills `2023-06-01` when absent.
...
Reads overview metrics, including `throughput`, `tokens`, `cost`, `host`, `cache`, `history`, and `pool`.

- `cache.cacheable_hit_rate` / `cache.cacheable_miss_rate` count only cacheable responses, so upstream failures and client cancellations do not pollute cache diagnostics.
- `cache.hit_rate` / `cache.miss_rate` remain total lookup ratios and include uncacheable failures for troubleshooting.
- `history.success_rate` excludes user-side `401 / 403 / 502 / 504 / 524`; these responses are counted under `history.excluded_from_failure_rate` and do not enter the failure-rate denominator.
...
- `X-DeepSeek-Web-To-API-Sample-Id`
- `X-DeepSeek-Web-To-API-Sample-Dir`
- `X-DeepSeek-Web-To-API-Sample-Meta`
- `X-DeepSeek-Web-To-API-Sample-Upstream`
...
Checks the current custom build version. It no longer calls the upstream GitHub Release API; update policy is self-managed by the current maintainer:
...
curl http://localhost:5001/v1/chat/completions \
  -H "Authorization: Bearer your-api-key" \
  -H "X-DeepSeek-Web-To-API-Target-Account: user@example.com" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "deepseek-v4-flash",
    "messages": [{"role": "user", "content": "Hello"}]
