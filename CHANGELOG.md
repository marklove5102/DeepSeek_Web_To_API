...
- **Thinking-Injection 提示词扩写工具链纪律**：[`internal/promptcompat/thinking_injection.go`](internal/promptcompat/thinking_injection.go) 的 `DefaultThinkingInjectionPrompt` 在原有"最大化推理"基础上追加 6 条工具链纪律（CALL / FORMAT / PARALLEL vs SEQUENTIAL / AFTER A RESULT / STOP / FAILURE MODES TO AVOID），直白告诉模型何时调用、如何调用、并行与串行如何拆分、读取结果后如何分支、何时收口、以及典型错误形态。其中 FORMAT 段显式警告 `<![CDATA|VALUE|]]>` 是无效写法（外层 wrapper 用 `|` 但 CDATA 必须用 `[`），从源头降低 CDATA 变体出现频率，与上面的解析器兼容形成"解析容错 + 提示纠正"双层防御。
